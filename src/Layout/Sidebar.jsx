...
const Sidebar = () => {
  return (
    <div
      className="lg:flex hidden flex-col bg-secondary w-60 p-3 
    ...
    >
      ...
      <div className="flex flex-col pt-5 gap-1 border-t border-lightText">
        {BOTTOM_SIDEBAR_LINKS.map((item, index) => (
          <SidebarLink key={index} item={item} />
        ))}
      </div>
      <button className="flex flex-row items-center gap-2 px-3 py-3 text-base rounded-sm hover:bg-indigo-50 active:bg-secondaryLight ">
        <span>
          <IoLogOut />
        </span>
        <h3>Logout</h3>
