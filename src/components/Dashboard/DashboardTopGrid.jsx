  ...
  return (
    <Container className="grid lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1 gap-3 w-full font-popins py-3">
      {DashboardTopGridItems.map((item, id) => (
        <div
          key={id}
          className="bg-primaryLight rounded-sm px-4 flex-1 flex items-center justify-start"
        >
          <div
            className={`${item.iconColor} p-3 rounded-full flex justify-center items-center`}
          >
            <FaShoppingBag size={20} color="white" />
          </div>
          <dir className="flex flex-col pl-4">
            <h2 className=" text-lightText lg:text-lg">{item.title}</h2>
            <div className="flex flex-row gap-2 items-center">
              <span className="text-xl font-semibold">{item.value}</span>
              <span
                className={`text-base ${
                  item.differenceBy === "+" ? "text-success" : "text-warning"
                }`}
              >
                {item.difference}
              </span>
